...
public class CursorTransactionContextSupplier implements CursorContextSupplier
{
    protected ThreadLocal<CursorContext> cursorContext;

    @Override
    public void init( LongSupplier lastClosedTransactionIdSupplier )
    {
        this.cursorContext = ThreadLocal.withInitial( () -> new CursorTransactionContext( lastClosedTransactionIdSupplier ) );
    }

    @Override
    public CursorContext getCursorContext()
    {
        return cursorContext == null ? EmptyCursorContext.INSTANCE : cursorContext.get();
    }
